%% Subplot confronto UKF
%Questo script è usato per l'UKF per graficare i risultati importanti
%ottenuti ed avviare la eventuale simulazione di confronto
%Dato che per implementare il filtro UKF si sono dovute fare medie di
%angoli, le variabili angolari stimate sono riportate stavolta in -pi; pi,
%così analogamente si limitano anche le variabili di giunto vere per un
%confronoto più adeguato (non cambia la stima è solo un metodo di
%rappresentazione)
teta1=atan2(sin(teta1),cos(teta1));
teta2=atan2(sin(teta2),cos(teta2));
